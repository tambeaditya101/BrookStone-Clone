...
const getData = () => {
  return fetch(` http://localhost:8080/massage`).then((res) => res.json());
};
const Massage = () => {
...
/></Center> : <Box className='container' display='flex' justifyContent='space-between' border='1px solid black'   w='85%' margin='auto' mt='30px' >
    ...
    <Box className="right-cont" w='75%' display='grid' gridTemplateColumns='repeat(3,1fr)' gap='20px' >
      {
        massage.map((i)=> <NavLink to={`/massage/${i.id}`} > <Box h='400px' boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' p='20px' >
